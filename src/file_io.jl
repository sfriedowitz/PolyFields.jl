...
"""
"""
function savefields(sys::FieldSystem, fpath::AbstractString; type::AbstractString = "omega")    
    # Open file handle
    io = open(fpath, "w")
...
function loadfields!(fpath::AbstractString)
    ...
    dims = Tuple(parse(Int, N) for N in split(sys_lines[1][11:end-1], ","))
    ...
    for mid in mids
        if field_type == "R"
            fields[mid] = zeros(Float64, dims)
        elseif field_type == "K"
            fields[mid] = zeros(Complex{Float64}, dims)
        end
    end
